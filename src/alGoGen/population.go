...
func (p *Population) computeProportions() (int, int) {
	...
	return totalNumOfOffspring, totalNumOfSelected
}

func (p *Population) computeOffspring(totalNumOfOffspring int, selectedIndividuals *shared.Individuals) *shared.Individuals {
...
func (p *Population) mutatePopulation(offspring *shared.Individuals, selectedNextIndividuals *shared.Individuals) *[]*shared.Individual {
...
func (p *Population) Run() int {
	...
	for !p.config.StoppingCriteria.Execute(i, p.individuals) {
		...
		totalNumOfOffspring, totalNumOfSelected := p.computeProportions()
		...
		offspring := *(p.computeOffspring(totalNumOfOffspring, &selectedIndividuals))
		...
		p.individuals = *(p.mutatePopulation(&offspring, &selectedNextIndividuals))
